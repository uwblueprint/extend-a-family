...
>(
  (
    { activity, onWrongAnswer, onCorrectAnswer, isCompleted, scale = 1 },
    ref,
  ) => {
      ...
      if (selectedLeft !== null) {
        setMatchesLeftToMid((prev) => {
          const filtered = prev.filter((pair) => {
            return (
              pair.leftRowIdx !== selectedLeft && pair.midRowIdx !== rowIdx
            );
          });
    ...
    const handleRightClick = (rowIdx?: number) => () => {
      ...
      setMatchesMidToRight((prev) => {
        const filtered = prev.filter((pair) => {
          return (
            pair.midRowIdx !== selectedMiddle && pair.rightRowIdx !== rowIdx
          );
        });
        return [
          ...filtered,
          { midRowIdx: selectedMiddle, rightRowIdx: rowIdx },
        ];
      });
      setSelectedMiddle(null);
    };
      ...
      const recalcLines = () => {
        ...
        const fromMidToRight = matchesMidToRight
          .map((pair) => {
            ...
            const y2 =
              (rightRect.top - containerRect.top + rightRect.height / 2) /
              scale;
                ...
                return (
                  ...
                  >
                    ...
                    <MatchingBox
                      ...
                      isMatched={
                        !!rightItem && isMiddleMatched(rightItem.rowIdx)
                      }
                      feedbackStatus={getMiddleFeedbackStatus(
                        rightItem?.rowIdx,
                      )}
