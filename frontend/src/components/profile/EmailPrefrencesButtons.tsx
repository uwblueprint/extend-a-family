...
const EmailPrefrencesButton: React.FC<EmailPrefrencesButtonProps> = ({setPrefrence, selectedId, buttonId, text}) => {
    const theme = useTheme();
    function emailOptionSelecttor() {
    if (selectedId === buttonId) {
      return {'backgroud': theme.palette.Facilitator.Light, 'border': theme.palette.Facilitator.Default}
    }
    return {'backgroud': theme.palette.Neutral[100], 'border': theme.palette.Neutral[600]}  
  }
    return (
        <Button 
        ...
        >
        {selectedId === buttonId? <RadioButtonChecked  sx={{
            paddingRight: '10px',
            height: '30px',
            width: '30px',
